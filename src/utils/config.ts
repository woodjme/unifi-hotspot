...
const config: Config = {
  ...
  unifiSiteName:
    process.env.UNIFI_SITENAME || process.env.SITENAME || 'default',
...
function validateConfig(): void {
  ...
  if (!config.uri) {
    logger.error(
      'Missing required environment variable: UNIFI_URL (or deprecated URI)',
    );
    process.exit(1);
  }
  if (process.env.URI) {
    logger.warn(
      'Warning: The URI environment variable is deprecated and will be removed in the future. Please use UNIFI_URL instead.',
    );
  }
  ...
  if (process.env.SITENAME) {
    logger.warn(
      'Warning: The SITENAME environment variable is deprecated and will be removed in the future. Please use UNIFI_SITENAME instead.',
    );
  }
